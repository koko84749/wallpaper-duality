...
~/Pictures/Live wall/        # Put .mp4 files here
~/Pictures/Wallpapers/       # Put .png/.jpg/.jpeg files here
```
...
Place `.mp4` files in `~/Pictures/Live wall/` and `.png/.jpg` files in `~/Pictures/Wallpapers/`.
...
- Change `MONITOR` in both scripts to match your display (find yours with `hyprctl monitors`)
- Change `WALLPAPER_DIR`/`STATIC_DIR` paths if your folders are different
...
- The wallpaper watcher (`wallpaper-watcher.sh`) runs as a systemd user service and sends a notification when new `.mp4`/`.png`/`.jpg` files are added
